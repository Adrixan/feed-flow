...
class DecSyncRepository internal constructor(
    ...
    fun restoreIfConfigured(context: Context) {
        val dirPath = decSyncSettings.getDirPath() ?: return
        setup(context, dirPath).onFailure {
            logger.e("DecSync: failed to restore setup from $dirPath", it)
        }
    }

    fun setup(context: Context, dirUri: String): Result<Unit> = runCatching {
        val instance: Decsync<Unit> = if (dirUri.startsWith("content://")) {
            val treeUri = Uri.parse(dirUri)
            val treeDocId = DocumentsContract.getTreeDocumentId(treeUri)
            val docUri = DocumentsContract.buildDocumentUriUsingTree(treeUri, treeDocId)
            Decsync(context, docUri, SYNC_TYPE, null, OWN_APP_ID)
        } else {
            val dir = File(dirUri)
            if (!dir.exists()) {
                dir.mkdirs()
            }
            Decsync(dir, SYNC_TYPE, null, OWN_APP_ID)
        }
        instance.addListeners()
        instance.initStoredEntries()
        decsync = instance
        decSyncSettings.setDirPath(dirUri)
        logger.d { "DecSync setup complete at: $dirUri" }
    }
    ...
    suspend fun syncFeedSources(): SyncResult = withContext(dispatcherProvider.io) {
        ...
        // Resolve names and categories into inserts
        ...
        // Push local feeds not yet in DecSync
